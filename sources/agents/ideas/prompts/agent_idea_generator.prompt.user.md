...
- **Technical Codename:** [You MUST dynamically translate the string literal token "Technical Codename" into the exact equivalent word of the requested target language "{% if language and language.strip() != "" %}{{ language }}{% else %}English{% endif %}". Then, generate and output a sharp, lean, clean single-word programming identifier token based on the idea title, strictly utilizing CamelCase or Kebab-case formatting rules without spaces, matching the code style shown in the visual reference, e.g., EcoMonitor, AutoSellHub, SecureRoute]
- **Brand Name:** [You MUST dynamically translate the string literal token "Brand Name" into the exact equivalent word of the requested target language "{% if language and language.strip() != "" %}{{ language }}{% else %}English{% endif %}". Then, formulate an attractive, professional commercial marketplace name that aligns perfectly with the technical codename, fully rendered in {% if language and language.strip() != "" %}{{ language }}{% else %}English{% endif %}]
...
# CRITICAL COMPLIANCE: You are STRICTLY FORBIDDEN from omitting this section. You MUST analyze the generated idea above and explode it into multiple precise, low-level technical requirement bullets. You MUST translate this entire section header label text into the exact equivalent wording of the requested target language "{% if language and language.strip() != "" %}{{ language }}{% else %}English{% endif %}". You MUST output at least 3 to 5 unique [REQ-XXX] lines, at least 1 to 2 [DAT-XXX] lines, and at least 3 to 5 [EXC-XXX] lines depending on the system engineering complexity.
# STRIKE BOUNDARY: All Tag IDs MUST be placed exactly at the start of the bullet line wrapped natively in single brackets and bold tags (e.g., `* **[REQ-001]** `). You are STRICTLY BANNED from referencing any specific programming file paths, physical directory scripts, or code components inside this document baseline, as file generation boundaries are reserved for the BA layer.
* **[REQ-001]** [Insert low-level atomic functional contract or webhook controller task description. Do NOT include any code file paths. Fully translate description text into {% if language and language.strip() != "" %}{{ language }}{% else %}English{% endif %}]
* **[REQ-002]** [Insert the next sequential functional requirements contract. Do NOT include any code file paths. Fully translate into {% if language and language.strip() != "" %}{{ language }}{% else %}English{% endif %}]
* **[DAT-001]** [Insert low-level flat data model entity or data persistence table directive. Do NOT include any code file paths. Fully translate into {% if language and language.strip() != "" %}{{ language }}{% else %}English{% endif %}]
* **[EXC-001]** [Insert low-level exception gate fallback validation directive. Do NOT include any code file paths. Fully translate into {% if language and language.strip() != "" %}{{ language }}{% else %}English{% endif %}]
