...
class EnterpriseAutonomousProjectEstimatorAgent(AbstractSubAgent):
    ...
    def clean_response(self, raw_response, **kwargs):
        ...
        if not mermaid_blocks:
            self.logger.warn("[ ℹ️ WARN ] Zero functional mermaid visualization blocks detected inside the markdown body.")
        ...
        for idx, code in enumerate(mermaid_blocks):
            ...
            self.logger.info(f"[ ⏳ RENDERING ] Querying dynamic vector graph data for layout file: {file_name}...")
            try:
                ...
                if img_response.status_code == 200 and b"<svg" in img_response.content:
                    ...
                    self.logger.info(f"[ 💾 IMAGE ] Highly-scalable vector graphic successfully exported to: {target_file}")
                else:
                    self.logger.warn(f"[ ⚠️ WARNING ] Render pipeline returned invalid metadata block structure at index: {idx}")
            except Exception as net_err:
                self.logger.warn(f"[ ⚠️ WARNING ] Network exception or latency spike during chart extraction: {str(net_err)}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--buffer-ratio", type=float, help="Estimation with buffer ratio. Ex: 1.5")
