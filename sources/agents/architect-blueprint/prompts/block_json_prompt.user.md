...
- **🚨 NO FOLDER OR PACKAGE PATHS ALLOWED IN COMPONENTS (ABSOLUTE HARD LIMIT):** You are STRINCENTLY BANNED from generating any sub-task object where the 'components' array field contains a raw folder directory name or stops at a Java package structural layer. 
  ...
  * If a predicted components path ends with a raw directory folder name or a package boundary node (meaning the last characters of the string do NOT form a valid code file dot-extension), you are CRITICALLY FORBIDDEN from carrying it over blindly into the payload.
  * You MUST immediately intercept that truncated path container, evaluate the sub-task 'desc' context, determine the assigned agent persona, and dynamically expand that folder into an array of explicit physical file targets ending with proper format extensions by strictly enforcing the 'ENTERPRISE LAYERED TOPOLOGY ENFORCEMENT LAW' and 'Hardened DevOps Resource Isolation Protocol' rules defined in your System Prompt:
    - For 'Docker' Agent: Explode generic infrastructure layers into distinct, service-level containerization targets mapped to active module roots (e.g., `"components": ["./sources/backend/auth-service/Dockerfile", "./sources/backend/user-service/Dockerfile", "./sources/frontend/web-app/Dockerfile"]`).
    ...
    - For 'GKE' Agent: Completely isolate and decouple Helm chart configurations (e.g., `./sources/infra/gke/charts/gateway/Chart.yaml`, `./sources/infra/gke/charts/gateway/values.yaml`) from runtime verification manifests (e.g., `./sources/infra/gke/manifests/hpa.yaml`, `./sources/infra/gke/manifests/ingress.yaml`).
    - For 'Doc' Agent: Explode generic folder paths into real project markdown documentation targets exclusively, completely excluding internal agent metadata file boundaries like `.ai/` or `.context/` paths.
  * Every single array node element string must satisfy this explicit terminal file extension filter to pass backend runtime validation.
  * You MUST immediately intercept that truncated path container, think step-by-step like a Senior Solution Architect, evaluate the sub-task 'desc' context, determine the designated agent role, and dynamically expand that plain folder path into an array of explicit physical file targets ending with proper code format suffixes by strictly applying the mandatory layered architecture topologies (Java source/test enterprise packages or frontend module layout standards) specified in your System Prompt rules. Every single array node element string must satisfy this explicit terminal file extension filter to pass backend runtime validation.
- 🚨 **MANDATORY HARDENED PROJECT-ONLY DOCUMENTATION ARTIFACT LAW (Enforced for Agent Doc)**:
  * The 'Doc' agent is strictly an engineering documentation persona and is ABSOLUTELY BANNED from outputting naked directories, folder paths, or system-level agent runtime contexts in the 'components' array.
  * You are CRITICALLY FORBIDDEN from including internal agent pipeline context blueprints, orchestration tracks, or metadata files (specifically BANNED from outputting files under `.ai/`, `.plan/`, or `.context/` paths like `phase-4.context.blueprint.md`). The technical documentation must focus exclusively on the actual project code asset artifacts.
  * Every string element inside the 'components' array for the 'Doc' agent MUST strictly point to a physical project markdown documentation file ending with the explicit dot extension `.md` (e.g., replace raw directory scopes like `.../mobile-app` with target documentation assets such as `./sources/frontend/mobile-app/README.md` or `./sources/frontend/mobile-app/ARCHITECTURE.md`).
  * If a calendar day contains technical directives or documentation requirements but lacks specific source file configurations, populate the 'components' array with dedicated architectural markdown file targets for that specific application domain (e.g., `./sources/backend/README.md`).
