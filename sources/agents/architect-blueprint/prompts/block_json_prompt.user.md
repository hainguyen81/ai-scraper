...
- **🚨 NO FOLDER OR PACKAGE PATHS ALLOWED IN COMPONENTS (ABSOLUTE HARD LIMIT):** You are STRINCENTLY BANNED from generating any sub-task object where the 'components' array field contains a raw folder directory name or stops at a Java package structural layer. 100% of all components elements MUST dynamically target an explicit physical FILE ending strictly with a functional format extension (such as `.java`, `.tf`, `.sql`, `.yml`, `.xml` or `Dockerfile`). If the raw baseline blueprint blueprint lists an ambiguous package or directory path (e.g., `.../studentcard`), you MUST programmatically dissect it and expand it into explicit logical target file components based on the assigned agent task requirement (e.g., for 'Coder' -> `.../studentcard/StudentCardService.java`; for 'Tester' -> `.../studentcard/StudentCardServiceTest.java`). Every element string must contain a literal dot extension.
