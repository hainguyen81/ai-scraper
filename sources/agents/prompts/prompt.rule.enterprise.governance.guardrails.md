...
- 🚨 **UNIVERSAL LAYOUT & HEADER LOCALIZATION PARADIGM (FORCED OVERRIDE)**: 
    ...
    * Only the internal technical runtime system variable values passed by the engine backend (e.g., `{{ project_name }}`, `ARCH-{{ doc_id }}`, `{{ current_timestamp }}`) MUST be preserved natively in pure Technical English. Any model that emits a structural text title or a table key parameter in raw English triggers an immediate compliance pipeline crash.
