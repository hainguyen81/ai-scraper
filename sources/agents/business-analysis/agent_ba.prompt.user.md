...
2. **Project Naming**: Propose 3 tailored project names (Technical Codename, Descriptive Name, and Brand Name) and populate them into the "project_names" keys. IF {{ project_name }} is provided (not empty), you MUST use that exact string for the "technical_codename" value without any changes.ELSE (if {{ project_name }} is blank or omitted), you are free to creatively generate a unique technical codename based on the raw idea.
