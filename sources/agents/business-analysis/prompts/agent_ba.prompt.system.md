...
- Database Tables, Column Definitions, Keys & Constraints: Use **[DAT-XXX]** (Precise types, nullability, PK/FK links).
  ...
  * **ANTI-CRASH MERMAID GRAMMAR LAWS:** You MUST rigorously enforce strict Mermaid.js erDiagram vocabulary rules. You are ABSOLUTELY BANNED from utilizing non-supported database constraints or corrupt data types inside the field rows.
    - **THE ZERO-GLOBAL-BRACKET MANDATE:** The declaration word `erDiagram` MUST stand completely isolated on the very first line of the code block. You are STRICTLY BANNED from opening or wrapping any global curly braces `{}` immediately following the `erDiagram` keyword or around the collective group of entities. Global nesting blocks are flatly prohibited. Curly braces `{}` are strictly restricted to the internal attribute definitions of individual isolated entities (e.g., `USERS { ... }`).
