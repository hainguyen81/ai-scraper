...
- Database Tables, Column Definitions, Keys & Constraints: Use **[DAT-XXX]** (Precise types, nullability, PK/FK links).
  ...
  * **ANTI-CRASH MERMAID GRAMMAR LAWS:** You MUST rigorously enforce strict Mermaid.js erDiagram vocabulary rules. You are ABSOLUTELY BANNED from utilizing non-supported database constraints or syntax structures inside the field rows. 
    ...
    - **THE ZERO-GHOST-TOKEN LIMITATION:** Every single line defining a field attribute MUST contain exactly two string tokens (type_name and field_name) or exactly three string tokens if a single valid constraint is applicable (e.g., type_name field_name PK/FK). You are STRICTLY BANNED from appending loose un-quoted characters, ghost text fragments, or corrupted token strings (e.g., raw text keys like "hars", "uniq") at the end of the data lines. Any extra descriptive metadata, parameter notes, or property indicators MUST be encapsulated entirely inside double quotes `""` at the absolute end of the field row to serve as a descriptive label string only (e.g., `varchar tax_id "UNIQUE, NOT NULL"`).
    - **THE RELATIONSHIP MANDATE:** You MUST explicitly output and define valid relational vector connectivity strings using strict cardinality operators (e.g., `||--o{`) at the absolute end of the erDiagram block. Leaving multiple block entities floating inside the graph canvas without active logical connectors is strictly prohibited.
