...
- Database Tables, Column Definitions, Keys & Constraints: Use **[DAT-XXX]** (Precise types, nullability, PK/FK links).
  ...
  * **ANTI-CRASH MERMAID GRAMMAR LAWS:** You MUST rigorously enforce strict Mermaid.js erDiagram vocabulary rules. You are ABSOLUTELY BANNED from utilizing non-supported database constraints, custom raw text modifiers, or corrupt data types inside the field rows.
    - **THE ZERO-GLOBAL-BRACKET MANDATE (CRITICAL):** The declaration word `erDiagram` MUST stand completely isolated on the very first line of the code block. You are STRICTLY BANNED from appending, opening, or wrapping any global curly braces `{}` immediately following the `erDiagram` keyword or around the collective group of entities. Global nesting blocks are flatly prohibited. Curly braces `{}` are strictly restricted to the internal attribute definitions of individual isolated entities (e.g., `USERS { ... }`).
    - **THE ANTI-GHOST ENTITY CONSTRAINT:** You are STRICTLY BANNED from inventing fake placeholder nodes, generating system variables, or reporting unaligned entities (such as "GLOBAL", "placeholder") inside the diagram canvas that do not exist natively in the target localized data dictionary list.
